...
type
  ...
  AstNode* = object
    ...
    token: TokenIndex
      ...
      ## originally parsed, if this node has a string.
  ...
  AstExtra* = seq[AstIndex]
    ## this sequence is used by `Ast` to lookup extra data that doesn't fit
    ## into `AstNode`'s `left` and `right` properties. Instead the `kind` tag
    ## on the `AstNode` will determine two things:
  ...
  AstNodeKind* {.pure.} = enum
    ...
    # nodes below this have no legacy mapping

    # compilation related - begin

    ankProject,      ## nimskull compilation project
    ankModule,       ## a module within a program

    # compilation related - finish
...
const
  ...
  unknownAstRight* = emptyAstRight
    ## empty and unkonwn are ambiguous, aliased to communicate intent
  nodesWithoutLegacyMapping = {ankProject, ankModule}

func nodes*(a: Ast): seq[AstNode] {.deprecated: "for debugging only".} =
  ## looking inside an `Ast`'s nodes
  a.nodes
func extra*(a: Ast): AstExtra {.deprecated: "for debugging only".} =
  ## looking inside an `Ast`'s nodes
  a.extra
...
func legacyNodeToAstKind*(n: PNode): AstNodeKind =
  ...
  assert result != nodesWithoutLegacyMapping
